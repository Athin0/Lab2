...
template<class T>
DynamicArray<T>::DynamicArray(T *items, int newSize) {
    arr = nullptr;
    size = 0;
    if (newSize < 0) return;           //создание нового пустого массива и копирование в него данных из исходного
    Resize(newSize);
...
template<class T>
int DynamicArray<T>::GetSize() {           //получить размер массива
    return size;
}
